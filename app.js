...
const bridgeLibrary = {
  ...
  stretch: [
    ["Explore", "Function research and structured online learning"],
    ["Experience", "Technical training, communities of practice and mentoring"],
    ["Stretch", "A supervised technical project or substantial stretch assignment"],
    ["Move", "Rotation or temporary assignment before a permanent move"]
  ],
  multi: [
...
function renderBridge(type) {
  $("#bridgeSteps").innerHTML = bridgeLibrary[type].map(([stage, action]) => `
...
function bankItems() {
  if (skillView === "master") {
    return MASTER_SKILLS.map(item => ({
      ...item, meta: "Transferable capability", functionName: "Shared across functions"
    }));
  }
  if (skillView === "department") {
    return DEPARTMENT_SKILLS.map(item => ({
      ...item, meta: "Department-wide skill", functionName: "Shared across CUDAR"
    }));
  }
  ...
  return functions.flatMap(fn => {
    ...
    if (defined.length) {
      return defined.map(item => ({
        ...item, meta: "Function-applied skill", functionName: fn
      }));
    }
    return [
      ...FUNCTIONS[fn].capabilities.map(name => ({
        name,
        definition: "Transferable capability used in this function.",
        meta: "Function-applied skill",
        functionName: fn
      })),
      ...FUNCTIONS[fn].specialist.map(name => ({
        name,
        definition: "Specialist knowledge listed for this function.",
...
function skillAppearances(name) {
  const matches = [];
  Object.entries(FUNCTIONS).forEach(([fnName, fnData]) => {
    const foundIn = [];
    if ((fnData.capabilities || []).some(s => s.toLowerCase() === name.toLowerCase())) foundIn.push("capability");
    if ((fnData.specialist || []).some(s => s.toLowerCase() === name.toLowerCase())) foundIn.push("specialist knowledge");
    if ((fnData.definedSkills || []).some(s => s.name.toLowerCase() === name.toLowerCase())) foundIn.push("defined skill");
    if (foundIn.length) matches.push({ functionName: fnName, matchTypes: foundIn });
  });
  return matches;
}
...
function renderSkillDetail(item) {
  ...
  const appearances = skillAppearances(item.name);
  const appearancesList = appearances.length
    ? appearances.map(entry => `<li>${escapeHTML(entry.functionName)} — ${escapeHTML(entry.matchTypes.join(", "))}</li>`).join("")
    : `<li>${escapeHTML(item.functionName)}</li>`;

  const examples = appearances.length > 1
    ? appearances.map(entry => `${escapeHTML(entry.functionName)}`).slice(0, 3).join(", ")
    : (item.functionName || "This function");

  detail.innerHTML = `
    <div class="skill-detail__meta">${escapeHTML(item.meta || "Skill")}</div>
    <h3>${escapeHTML(item.name)}</h3>
    <p>${escapeHTML(item.definition)}</p>
    <div class="skill-detail__body">
      <div>
        <h4>Where it appears</h4>
        <ul>${appearancesList}</ul>
      </div>
      <div>
        <h4>How it is applied</h4>
        <ul>
          <li>Used in ${escapeHTML(examples)}.</li>
          <li>The same underlying capability can translate to different contexts, processes and language depending on the function.</li>
        </ul>
      </div>
    </div>
  `;
...
function renderSkillsBank() {
  ...
  let items = bankItems();
  if (skillView !== "function" && selectedFunction !== "all") {
  ...
  if (!items.length) {
    ...
    $("#skillDetail").innerHTML = `
      <div class="eyebrow eyebrow--dark">Skill detail</div>
      <h3>No matching skill selected.</h3>
      <p>Try a broader search or choose a different function view.</p>
    `;
    return;
  }

  grid.innerHTML = items.map((item, index) => `
    <article class="skill-card" data-skill-name="${escapeHTML(item.name)}" tabindex="0" role="button" aria-label="Open details for ${escapeHTML(item.name)}">
  ...
  cards.forEach(card => {
    card.addEventListener("click", () => {
      $$(".skill-card").forEach(el => el.classList.remove("is-selected"));
      card.classList.add("is-selected");
      const cardData = items.find(item => item.name === card.dataset.skillName);
      if (cardData) renderSkillDetail(cardData);
    });
